...
export default function WISARMaintenanceApp() {
  ...
  function getVehicleMaintLogs(vehicleId: string) {
    return maintenanceLogs.filter(l => l.vehicle_id === vehicleId)
  }

  function getVehicleInspections(vehicleId: string) {
    return inspectionLogs.filter(l => l.vehicle_id === vehicleId)
  }
          ...
          <div>
            ...
            <div className="grid gap-3">
              {vehicles.map(vehicle => {
                ...
                const logs = getVehicleMaintLogs(vehicle.id)
                const inspections = getVehicleInspections(vehicle.id)
                return (
                  <div
                    key={vehicle.id}
          ...
          <div>
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-4">
              ...
              <div className="grid gap-3">
                ...
                <input type="date" placeholder="Next service date" value={maintenanceForm.next_service} onChange={e => setMaintenanceForm(p => ({...p, next_service: e.target.value}))} className="border rounded p-2" />
                <button
                  onClick={addMaintenanceLog}
                  disabled={saving || !maintenanceForm.description || !maintenanceForm.technician}
            ...
            <div>
              <h3 className="font-semibold mb-2">
                History for {selectedVehicleData.name}
              </h3>
              {getVehicleMaintLogs(selectedVehicle).length === 0 ? (
                <p className="text-gray-500 text-sm">No maintenance records yet.</p>
              ) : (
                getVehicleMaintLogs(selectedVehicle).map(log => (
                  <div key={log.id} className="bg-white rounded shadow p-3 mb-2">
                    <div className="flex justify-between">
                      <span className="font-medium">{log.type.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}</span>
        ...
        {dashboardView === 'inspection' && (
          <div>
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-4">
              ...
              <div className="grid gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => setInspectionForm(p => ({...p, passed: !p.passed}))}
                ...
                />
                <button
                  onClick={addInspectionLog}
                  disabled={saving || !inspectionForm.driver}
                  className="bg-blue-900 text-white py-2 rounded disabled:opacity-50 font-semibold"
                >
            ...
            <div>
              ...
              {getVehicleInspections(selectedVehicle).length === 0 ? (
                <p className="text-gray-500 text-sm">No inspection records yet.</p>
              ) : (
                getVehicleInspections(selectedVehicle).map(log => (
                  <div key={log.id} className="bg-white rounded shadow p-3 mb-2">
                    <div className="flex justify-between items-center">
                      <span className="font-medium">Week of {log.week}</span>
                      <span className={`text-sm font-bold ${log.passed ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div>
            ...
            <div className="bg-white rounded-lg shadow p-4 mb-4">
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={addDriver} className="bg-blue-900 text-white px-4 rounded font-semibold">
                  Add
                </button>
              </div>
            </div>
