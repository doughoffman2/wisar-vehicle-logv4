...
const vehicles = [
  { id: 'pump-truck', name: 'Large Pump Truck - BIG RED', type: 'fire', icon: Truck, status: 'operational', hasPump: true, lastTest: '2026-01-01' },
  { id: 'kubota-fire', name: 'Kubota Fire Truck', type: 'fire', icon: Truck, status: 'operational', hasPump: true, lastTest: '2026-01-01' },
  { id: 'gator-fire', name: 'Gator Fire Truck', type: 'fire', icon: Truck, status: 'operational', hasPump: true, lastTest: '2026-01-01' },
  { id: 'ambulance-2018', name: '2018 Kubota Ambulance', type: 'ems', icon: Cross, status: 'operational', hasPump: false, lastTest: '2026-01-01' },
  { id: 'ambulance-new', name: 'New Ambulance (2024)', type: 'ems', icon: Cross, status: 'operational', hasPump: false, lastTest: '2026-01-01' },
  { id: 'boat-rescue', name: 'Rescue Boat', type: 'marine', icon: Ship, status: 'operational', hasPump: false, lastTest: '2026-01-01' },
  { id: 'utv-rescue', name: 'UTV Rescue', type: 'rescue', icon: Truck, status: 'operational', hasPump: false, lastTest: '2026-01-01' },
  ]
...
export default function WISARMaintenanceApp() {
  ...
  async function addInspectionLog() {
        ...
        const newLog: InspectionLog = {
                vehicle_id: selectedVehicle,
                date: new Date().toISOString().split('T')[0],
                driver: inspectionForm.driver,
                week: currentWeek,
                notes: inspectionForm.notes || undefined,
                passed: inspectionForm.passed
        }
        ...
        return (
                <div className="min-h-screen bg-gray-100 flex items-center justify-center">
                        <div className="text-center">
                                  <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-900 mx-auto mb-4"></div>div>
                                  <p className="text-gray-600">Loading WISAR Vehicle Data...</p>p>
                        </div>div>
                </div>div>
              )
  }
          ...
          <div className="min-h-screen bg-gray-100">
                <div className="bg-blue-900 text-white p-4">
                        <div className="flex items-center justify-center gap-2 mb-2">
                                  ...
                                  <h1 className="text-2xl font-bold">WISAR Vehicle Log</h1>h1>
                        </div>div>
                        <p className="text-center text-blue-200 text-sm">Water Island S&R</p>p>
                ...
                <div className="p-4 max-w-4xl mx-auto">
                                                                                                                                                      ...
                                                                                                                                                      <div key={vehicle.id} className="bg-white rounded-lg shadow p-4 cursor-pointer hover:shadow-md transition"
                                                                                                                                                                          ...
                                                                                                                                                                          <div className="flex items-center justify-between">
                                                                                                                                                                                                <div className="flex items-center gap-2">
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        <span className="font-semibold">{vehicle.name}</span>span>
                                                                                                                                                                                                                      </div>div>
                                                                                                                                                                                                <div className="text-right">
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        <span className="text-green-600 font-bold flex items-center gap-1">
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        {vehicle.hasPump && <div className="text-xs text-gray-500">PUMP</div>div>}
                                                                                                                                                                                                                      </div>div>
                                                                                                                                                                            </div>div>
                                                                                                                                                                          <div className="mt-2 text-sm text-gray-500">
                                                                                                                                                                                                <span>Last Test: {lastTest}</span>span>
                                                                                                                                                                                                <span className="ml-4">{logs.length} maintenance records</span>span>
                                                                                                                                                                                                <span className="ml-4">{inspections.length} inspections</span>span>
                                                                                                                                                                            </div>div>
                                                                                                                                                        </div>div>
                                                                                                                                                    )
                                    })}
                  ...
                  {dashboardView === 'maintenance' && (
                      <div>
                                  <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Wrench className="h-5 w-5" />Maintenance Log</h2>h2>
                                  <div className="mb-4">
                                                <label className="block text-sm font-medium mb-1">Vehicle</label>label>
                                                <select className="w-full p-2 border rounded" value={selectedVehicle} onChange={e => setSelectedVehicle(e.target.value)}>
                                                  {vehicles.map(v => <option key={v.id} value={v.id}>{v.name}</option>option>)}
                                                </select>select>
                                  </div>div>
                                  <div className="bg-white rounded-lg shadow p-4 mb-4">
                                                <h3 className="font-semibold mb-3">Add Maintenance Record</h3>h3>
                                                <div className="grid gap-3">
                                                                ...
                                                                <select value={maintenanceForm.type} onChange={e => setMaintenanceForm(p => ({...p, type: e.target.value}))} className="border rounded p-2">
                                                                                  <option value="routine">Routine Service</option>option>
                                                                                  <option value="repair">Repair</option>option>
                                                                                  <option value="inspection">Inspection</option>option>
                                                                                  <option value="pump-test">Pump Test</option>option>
                                                                                  <option value="oil">Oil Change</option>option>
                                                                                  <option value="tire">Tire Service</option>option>
                                                                                  <option value="other">Other</option>option>
                                                                </select>select>
                                                                <textarea placeholder="Description..." value={maintenanceForm.description} onChange={e => setMaintenanceForm(p => ({...p, description: e.target.value}))} className="border rounded p-2 h-20" />
                                                                <select value={maintenanceForm.technician} onChange={e => setMaintenanceForm(p => ({...p, technician: e.target.value}))} className="border rounded p-2">
                                                                                  <option value="">Select Technician...</option>option>
                                                                  {drivers.map(d => <option key={d} value={d}>{d}</option>option>)}
                                                                </select>select>
                                                                <input type="number" placeholder="Mileage/Hours (optional)" value={maintenanceForm.mileage} onChange={e => setMaintenanceForm(p => ({...p, mileage: e.target.value}))} className="border rounded p-2" />
                                                                <input type="date" value={maintenanceForm.next_service} onChange={e => setMaintenanceForm(p => ({...p, next_service: e.target.value}))} className="border rounded p-2" />
                                  ...
                                  <h3 className="font-semibold mb-2">History for {selectedVehicleData.name}</h3>h3>
                        {maintenanceLogs.filter(l => l.vehicle_id === selectedVehicle).length === 0 ? (
                                      <p className="text-gray-500 text-sm">No maintenance records yet.</p>p>
                                      ...
                                      <div key={log.id} className="bg-white rounded shadow p-3 mb-2">
                                                      <div className="flex justify-between">
                                                                        <span className="font-medium capitalize">{log.type.replace('-', ' ')}</span>span>
                                                                        <span className="text-sm text-gray-500">{log.date}</span>span>
                                                      </div>div>
                                                      <p className="text-sm mt-1">{log.description}</p>p>
                                                      <p className="text-xs text-gray-400 mt-1">Tech: {log.technician}{log.mileage ? ' | ' + log.mileage + ' hrs/mi' : ''}</p>p>
                                      </div>div>
                                    ))}
                      </div>div>
                        )}
                  ...
                  {dashboardView === 'inspection' && (
                      <div>
                                  <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><FileText className="h-5 w-5" />Weekly Inspections</h2>h2>
                                  <div className="mb-4">
                                                <label className="block text-sm font-medium mb-1">Vehicle</label>label>
                                                <select className="w-full p-2 border rounded" value={selectedVehicle} onChange={e => setSelectedVehicle(e.target.value)}>
                                                  {vehicles.map(v => <option key={v.id} value={v.id}>{v.name}</option>option>)}
                                  ...
                                  <div className="mb-4">
                                                <label className="block text-sm font-medium mb-1">Week of</label>label>
                                                <input type="date" value={currentWeek} onChange={e => setCurrentWeek(e.target.value)} className="border rounded p-2 w-full" />
                                  </div>div>
                                  <div className="bg-white rounded-lg shadow p-4 mb-4">
                                                <h3 className="font-semibold mb-3">Log Weekly Inspection</h3>h3>
                                                <div className="grid gap-3">
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                                  <label className="font-medium">Passed?</label>label>
                                                                ...
                                                                <textarea placeholder="Notes (optional)..." value={inspectionForm.notes} onChange={e => setInspectionForm(p => ({...p, notes: e.target.value}))} className="border rounded p-2 h-16" />
                                  ...
                                  <h3 className="font-semibold mb-2">Inspection History for {selectedVehicleData.name}</h3>h3>
                        {inspectionLogs.filter(l => l.vehicle_id === selectedVehicle).length === 0 ? (
                                      <p className="text-gray-500 text-sm">No inspection records yet.</p>p>
                                    ) : inspectionLogs.filter(l => l.vehicle_id === selectedVehicle).map(log => (
                                      <div key={log.id} className="bg-white rounded shadow p-3 mb-2">
                                                      <div className="flex justify-between items-center">
                                                                        <span className="font-medium">Week of {log.week}</span>span>
                                                                        <span className={`text-sm font-bold ${log.passed ? 'text-green-600' : 'text-red-600'}`}>{log.passed ? '✓ PASS' : '✗ FAIL'}</span>span>
                                                      </div>div>
                                                      <p className="text-xs text-gray-500">Driver: {log.driver} | Date: {log.date}</p>p>
                                        {log.notes && <p className="text-sm mt-1">{log.notes}</p>p>}
                                      </div>div>
                                    ))}
                      </div>div>
                        )}
                  ...
                  {dashboardView === 'drivers' && (
                      <div>
                                  <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Users className="h-5 w-5" />Driver Management</h2>h2>
                                  <div className="bg-white rounded-lg shadow p-4 mb-4">
                                                <h3 className="font-semibold mb-3">Add Driver</h3>h3>
                                                <div className="flex gap-2">
                                                                <input type="text" placeholder="Driver name..." value={newDriverName} onChange={e => setNewDriverName(e.target.value)}
                                                                                    onKeyDown={e => e.key === 'Enter' && addDriver()} className="flex-1 border rounded p-2" />
                                                                <button onClick={addDriver} className="bg-blue-900 text-white px-4 rounded font-semibold">Add</button>button>
                                                </div>div>
                                  </div>div>
                                  <div className="bg-white rounded-lg shadow p-4">
                                                <h3 className="font-semibold mb-3">Current Drivers ({drivers.length})</h3>h3>
                                                <div className="grid grid-cols-2 gap-2">
                                                  {drivers.map(driver => (
                                          <div key={driver} className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                                                              <Users className="h-4 w-4 text-blue-800" />
                                                              <span>{driver}</span>span>
                                          </div>div>
                                        ))}
                                                </div>div>
                                  </div>div>
